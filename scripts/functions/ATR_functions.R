...
get_GSEA_stat <- function(DE_results, isMice=F, MGI_to_HGNC=NULL,isProt=F){
  res_proc<- DE_results
  if(isProt){
...
# Plot volcano
plot_volcano<- function(DE_results, gene, isProt=F, useHyperbolicTH=F, labelAll=F, labelCol="red", p_cu=0.01, logFC_cu=2, curve=0.5, plotTH=T, plot_nominal_p=F, th_nominal_p=F){
